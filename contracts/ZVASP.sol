...
contract ZVASP is Ownable, ReentrancyGuard {
    ...
    event RewardPaid(uint256 indexed surveyId, address indexed respondent, uint256 amount);
    ...
    event FundsWithdrawn(uint256 indexed surveyId, address indexed creator, uint256 amount);
    ...
    constructor(address _treasuryAddress, address _relayServer) Ownable(msg.sender) {
    ...
    function batchMarkUsersVerified(address[] calldata users) external onlyRelay {
    ...
    function calculateCommission(uint256 totalFunding) public pure returns (uint256) {
    ...
    function calculateFromTotalFunding(uint256 totalFunding, uint256 maxResponses) 
        public 
        pure 
        returns (uint256 commission, uint256 availableForRewards, uint256 rewardPerResponse) 
    {
    ...
    function calculateFromIndividualAmount(uint256 rewardPerResponse, uint256 maxResponses) 
        public 
        pure 
        returns (uint256 commission, uint256 totalFunding) 
    {
        require(rewardPerResponse > 0 && maxResponses > 0, "Invalid parameters");
        ...
        uint256 totalNeeded = (totalRewards * BASIS_POINTS) / (BASIS_POINTS - COMMISSION_RATE);
    ...
    function createSurveyWithTotalFunding(
        ...
        (uint256 commission, uint256 availableForRewards, uint256 rewardPerResponse) = 
            calculateFromTotalFunding(totalFunding, maxResponses);
        
        require(msg.value >= totalFunding, "Insufficient payment");
        
        _createSurvey(_title, _description, _questions, totalFunding, rewardPerResponse, commission, availableForRewards, maxResponses);
    ...
    function createSurveyWithIndividualAmount(
        ...
        (uint256 commission, uint256 totalFunding) = 
    ...
    function _createSurvey(
        ...
        emit SurveyCreated(surveyCount, msg.sender, _title, totalFunding, rewardPerResponse, commission, maxResponses);
    ...
    function submitResponse(
        ...
        require(!surveys[_surveyId].hasResponded[msg.sender], "Already responded to this survey");
        ...
        require(s.availableFunding >= s.rewardPerResponse, "Insufficient funds in survey pool");
        ...
        for (uint i = 0; i < s.questions.length; i++) {
            if (_isMCQOrRating(s.questions[i].questionType)) {
                ...
                if (answer > 0) {
                    require(answer <= s.questions[i].options.length, "Invalid MCQ option");
    ...
    // View functions
    function getSurvey(uint256 _surveyId) external view returns (SurveyView memory) {
        require(_surveyId <= surveyCount && _surveyId > 0, "Invalid survey ID");
        Survey storage s = surveys[_surveyId];
    ...
    function getQuestions(uint256 _surveyId) external view returns (Question[] memory) {
        require(_surveyId <= surveyCount && _surveyId > 0, "Invalid survey ID");
        return surveys[_surveyId].questions;
    }
    
    function getAnswerCounts(uint256 _surveyId, uint256 _questionIndex) external view returns (uint256[] memory) {
        require(_surveyId <= surveyCount && _surveyId > 0, "Invalid survey ID");
        Survey storage s = surveys[_surveyId];
        require(_questionIndex < s.questions.length, "Invalid question index");
        
        uint256[] memory counts = new uint256[](s.questions[_questionIndex].options.length);
        for (uint i = 0; i < s.questions[_questionIndex].options.length; i++) {
            counts[i] = s.answerCounts[_surveyId][_questionIndex][i];
        }
        return counts;
    }
    
    function getSubjectiveResponses(uint256 _surveyId, uint256 _questionIndex) external view returns (string[] memory) {
        require(_surveyId <= surveyCount && _surveyId > 0, "Invalid survey ID");
